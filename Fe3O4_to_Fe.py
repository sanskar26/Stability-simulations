...
def fuc(t, c_1, c_2, poww):
    ...
    # This Function Returns PH2/(pH2+ PH2O)
...
fig = px.scatter(x=li_t, y=fuc(li_t, 89.2, 101800, 4),)
fig.update_traces(marker=dict(size=10, color="green"),
                  name=r"$Fe_3O_4  \ to \ Fe$", showlegend=True)

fig.update_layout(title=r'$Stability \ Curve \ For \ Fe_3O_4 \ and \ Fe$',
...
fig.add_annotation(text=r"$Fe_3O_4 \ is \ stable$",
                   ...
                   x=0.25, y=0.20, showarrow=False, font=dict(family='Gravitas One, cursive',
                                                              size=20,
                                                              color='MidnightBlue'))
fig.add_annotation(text=r"$Fe \ is \ stable$",
                   xref="paper", yref="paper",
